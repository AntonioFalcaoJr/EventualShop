...
public record CatalogItem(Guid Id, Guid CatalogId, Guid InventoryId, Product Product, string Sku, int Quantity)
    : ValueObject<CatalogItemValidator>
{
    public static implicit operator CatalogItem(Dto.CatalogItem item)
        => new(item.Id, item.CatalogId, item.InventoryId, item.Product, item.Sku, item.Quantity);
