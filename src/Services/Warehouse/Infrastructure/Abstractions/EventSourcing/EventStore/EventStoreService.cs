    ...
    where TId : struct
{
    ...
    {
        var sendEndpoint = await _bus.GetSendEndpoint(new Uri($"queue:warehouse.{KebabCaseEndpointNameFormatter.Instance.SanitizeName(message.GetType().Name)}.business-error"));
