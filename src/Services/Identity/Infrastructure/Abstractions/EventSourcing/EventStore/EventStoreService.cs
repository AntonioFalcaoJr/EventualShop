    ...
    where TId : struct
{
    ...
    {
        var sendEndpoint = await _bus.GetSendEndpoint(new Uri($"queue:identity.{KebabCaseEndpointNameFormatter.Instance.SanitizeName(message.GetType().Name)}.business-error"));
