    ...
    where TId : struct
{
    ...
    {
        var sendEndpoint = await _bus.GetSendEndpoint(new Uri($"queue:account.{KebabCaseEndpointNameFormatter.Instance.SanitizeName(message.GetType().Name)}.business-error"));
