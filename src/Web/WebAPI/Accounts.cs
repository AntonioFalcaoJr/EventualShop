...
public static class Accounts
{
    ...
    private static Task<Accepted> CreateAsync(IBus bus, Request.CreateAccount request, CancellationToken ct)
    ...
    private static Task<Accepted> DeleteAsync(IBus bus, Guid accountId, CancellationToken ct)
        => ApplicationApi.SendCommandAsync<Command.DeleteAccount>(bus, new(accountId), ct);

    private static Task<Accepted> DefineProfessionalAddressAsync(IBus bus, Guid accountId, Dto.Address address, CancellationToken ct)
        => ApplicationApi.SendCommandAsync<Command.AddShippingAddress>(bus, new(accountId, address), ct);

    private static Task<Accepted> DefineResidenceAddressAsync(IBus bus, Guid accountId, Dto.Address address, CancellationToken ct)
