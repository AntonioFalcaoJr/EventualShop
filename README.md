# EDA.CleanArch.DDD.CQRS.EventSourcing (WIP - work in progress)
...
This project applies the EventStorming workshop to decompose and aggregate correlated business capabilities in **Bounded Contexts** of a simple e-commerce, as well as the **Domain Events** (business facts) that provide integration between them. In addition to demonstrating implementation under a reactive **Event-driven architecture** (EDA), through the **event-sourcing** design, supported by the **CQRS** pattern, in a **Clean Architecture**.
